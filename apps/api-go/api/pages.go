...
func serveRegister(w http.ResponseWriter, r *http.Request) {
	http.ServeFile(w, r, "views/pages/register.html")
}
...
<div class="dashboard">
    ...
    <div class="stats-grid">
        <div class="stat-card"><span class="stat-value">%v XP</span><span class="stat-label">Total XP</span></div>
        <div class="stat-card"><span class="stat-value">#%v</span><span class="stat-label">Rang</span></div>
    </div>
    <h2>Quiz populaires</h2>
    <div class="quiz-grid">%s</div>
</div>`,
		getDisplayName(user),
		getMapInt(user, "total_xp"),
		user["rank"],
...
<div class="explore">
    ...
    <div class="quiz-grid">%s</div>
</div>`, catTags, renderQuizCards(quizzes))
...
func serveLeaderboard(w http.ResponseWriter, r *http.Request, user map[string]interface{}) {
	...
	for i, e := range entries {
		...
		rows += fmt.Sprintf(`<div class="lb-row"><span class="lb-rank">%s</span><span class="lb-name">%s</span><span class="lb-xp">%v XP</span></div>`,
			medal, getMapStr(e, "username"), getMapInt(e, "total_xp"))
	}
...
func serveProfile(w http.ResponseWriter, r *http.Request, user map[string]interface{}) {
	...
	content := fmt.Sprintf(`
<div class="profile"><h1>👤 %s</h1>
<p class="bio">%s</p>
<div class="stats"><span>%d XP</span><span class="rank rank-%s">%s</span></div>
</div>`, nickname, bio, xp, strings.ToLower(rank), rank)
...
func serveMiniCup(w http.ResponseWriter, r *http.Request, user map[string]interface{}) {
	...
	content := miniCupPageHTML()
	renderPage(w, "Mini Cup", content, user)
}
...
func servePersonalQuizDashboard(w http.ResponseWriter, r *http.Request, user map[string]interface{}) {
	...
	for _, q := range quizzes {
		rows += fmt.Sprintf(`<div class="quiz-card"><h3>%s</h3><p>%s</p><a href="/personal-quiz/%s/edit" class="btn-primary">Modifier</a></div>`,
			getMapStr(q, "title"), getMapStr(q, "status"), getMapStr(q, "id"))
	}
	if rows == "" {
		rows = `<p>Aucun quiz. <a href="/personal-quiz/create">Créer un quiz</a></p>`
	}

	content := fmt.Sprintf(`<div><h1>🎯 Mon Quiz Personnel</h1>%s</div>`, rows)
	renderPage(w, "Mon Quiz Personnel", content, user)
}
...
func servePersonalQuizCreate(w http.ResponseWriter, r *http.Request, user map[string]interface{}) {
	...
	nickname := getDisplayName(user)
	content := personalQuizCreateHTML(nickname)
	renderPage(w, "Créer mon quiz", content, user)
}
...
func serveAuthSessionAPI(w http.ResponseWriter, r *http.Request) {
	jsonResponse(w, 200, map[string]string{"status": "ok"})
}
...
func renderQuizCards(quizzes []map[string]interface{}) string {
	html := ""
	for _, q := range quizzes {
		title := getMapStr(q, "title")
		series := getMapStr(q, "series")
		id := getMapStr(q, "id")
		pc := getMapInt(q, "play_count")
		qc := getMapInt(q, "question_count")
		html += fmt.Sprintf(`<div class="quiz-card"><div class="quiz-header">%s</div><h3>%s</h3><p>%d questions • %d plays</p><a href="/quiz/%s/play" class="btn-primary">Jouer</a></div>`,
			series, title, qc, pc, id)
	}
	return html
}
...
func getDisplayName(user map[string]interface{}) string {
	...
	if nn, ok := user["nickname"].(string); ok && nn != "" {
		return nn
	}
...
func personalQuizCreateHTML(nickname string) string {
	return fmt.Sprintf(`<div><h1>Crée ton quiz personnel</h1><p>Salut %s !</p></div>`, template.HTMLEscapeString(nickname))
}

func miniCupPageHTML() string {
	return `<div><h1>⚽ Mini Cup</h1><p>Le jeu de tir aux buts !</p></div>`
}
